...
The operator invokes the fixed scenario manually as root with `sudo -- nix
develop --command uv run --locked wireproof lab frr-smoke clab-ebgp-v4 --repeat
1`; the CLI never invokes `sudo` or elevates privileges. A second iteration
starts only after the first has cleaned up successfully. This smoke result is
